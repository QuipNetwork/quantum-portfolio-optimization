...
class ReturnsClusterer(BaseClusterer):
    """
    Cluster assets by return similarity.

    Groups assets with similar mean returns together.
    Useful for separating growth vs value stocks.
    """
    ...
    def compute_distance_matrix(self, returns: pd.DataFrame) -> np.ndarray:
        """
        Compute distance matrix based on mean returns.
        ...
        mean_returns = returns.mean(axis=0).values.reshape(-1, 1)

        # Euclidean distance between mean returns
        distance = pdist(mean_returns, metric='euclidean')
