...
def run_benchmark(
    ...
    # 19. Pasqal-Pulser-DMM — weighted MIS on a conflict-graph DMM register (n<=10)
    if _HAS_PASQAL:
        start = time.perf_counter()
        pasqal_optimizer = PasqalPortfolioOptimizer(
            assets=assets, **pasqal_constraints
...
def print_results_table(results: List[BenchmarkResult], title: str = "Benchmark Results"):
    ...
    # Header
    header = f"{'Solver':<24} {'Score':>8} {'Gap%':>7} {'Price':>8} {'Dur':>6} {'#':>3} {'Budget':>8} {'Dur':>6} {'Card':>6} {'Feas':>6} {'Time(ms)':>20}"
    print(header)
    print("-" * 120)
